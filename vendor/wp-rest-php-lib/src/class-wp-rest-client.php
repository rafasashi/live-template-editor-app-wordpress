...
abstract class WP_API_REST_Client {
	...
	public function __construct() {
		$this->set_api_transport( new WP_API_REST_Transport_Curl ); 
	}

	protected abstract function authenticate_request( WP_API_REST_Request &$request );	
	...
	public function send_api_request( $path, $method, $params = array(), $post_data = array(), $headers = array(), $is_multipart = false ) {
		...
		$request = new WP_API_REST_Request( $url, $method, $post_data, $headers, $is_multipart );
	...
	protected function send_request( WP_API_REST_Request $request ) {
		// TODO: move this to WP_API_REST_Request?
	...
	public function set_api_transport( WP_API_REST_Transport $transport ) {
